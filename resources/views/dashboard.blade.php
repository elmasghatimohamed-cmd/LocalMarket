...
@section('content')
    ...
    const statusLabel = parentTd.querySelector('.role-change-status');
    const currentRoleLabel = parentTd.querySelector('span.text-white\\/50');
    const toast = document.getElementById('role-change-toast');

    submitBtn.classList.add('animate-spin', 'text-protech');
    ...
    if (currentRoleLabel) {
    currentRoleLabel.innerText = 'Current: ' + (
    data.role.charAt(0).toUpperCase() + data.role.slice(1)
    );
    currentRoleLabel.classList.add('text-protech');
    setTimeout(() => currentRoleLabel.classList.remove('text-protech'), 2000);
    }

    if (toast) {
    toast.innerText = 'RÔLE MIS À JOUR : ' + data.role.toUpperCase();
    toast.classList.remove('hidden');
    setTimeout(() => toast.classList.add('hidden'), 3000);
    }

    statusLabel.innerText = 'SUCCESS';
    statusLabel.classList.add('text-protech');
    ...
    submitBtn.classList.remove('animate-spin', 'text-protech');
