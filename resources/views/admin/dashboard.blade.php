    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 mt-2">${{ number_format($stats['total_revenue'], 2) }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-8 mb-12">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('admin.categories.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium text-sm">+ Add Category</a>
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 bg-slate-200 text-slate-900 rounded-lg hover:bg-slate-300 transition font-medium text-sm">Manage Categories</a>
                <a href="{{ route('admin.role_switcher') }}" class="px-4 py-2 bg-slate-200 text-slate-900 rounded-lg hover:bg-slate-300 transition font-medium text-sm">All Users Roles</a>
                <button id="apply-role-changes" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed" disabled style="display:none;">✓ Apply Changes</button>
                    ...
                    <tbody>
                        @foreach($recent_orders as $order)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                                ...
                                <td class="py-3 px-4 text-slate-900 font-semibold">${{ number_format($order->total, 2) }}</td>
                                <td class="py-3 px-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.role_switcher') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">Manage All →</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                            ...
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                                ...
                                <td class="py-3 px-4">
                                    <form method="POST" action="{{ route('admin.role_switcher.update', $user) }}" class="flex items-center gap-2 role-change-form">
                                        @csrf
                                        <select name="role" class="role-select border border-slate-300 rounded-lg px-3 py-2 text-sm font-medium bg-white hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            @foreach($roles as $role)
                                                <option value="{{ $role }}" {{ $user->hasRole($role) ? 'selected' : '' }}>{{ ucfirst($role) }}</option>
                                            @endforeach
                                        </select>
                                        <button type="submit" class="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium">Save</button>
                                    </form>
                                    <div class="mt-2 role-change-status text-xs text-green-600 font-medium" style="display:none;"></div>
                                    <div class="mt-2">
                                        <span class="px-3 py-1.5 rounded-full text-xs font-semibold inline-block bg-slate-200 text-slate-700">
