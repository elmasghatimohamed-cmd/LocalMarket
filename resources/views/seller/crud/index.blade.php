    ...
    <div class="max-w-6xl mx-auto">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
            <div>
                ...
                <p class="text-gray-400 text-sm mt-1">Welcome back! Managing <span
                        class="text-white font-bold">{{ $products->count() }}</span> listings.</p>
            </div>
        ...
        <section class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
            <div class="bg-[#141414] border border-[#222] p-5 rounded-2xl flex items-center gap-4">
                <div class="bg-[#DFFF00]/5 p-3 rounded-xl text-[#DFFF00]"><i class="fas fa-box-archive text-xl"></i>
                </div>
                <div><span class="text-[10px] uppercase tracking-widest text-gray-500 block">Total Items</span><b
                        class="text-xl">{{ $products->count() }}</b></div>
            </div>
            <div class="bg-[#141414] border border-[#222] p-5 rounded-2xl flex items-center gap-4">
                <div class="bg-[#DFFF00]/5 p-3 rounded-xl text-[#DFFF00]"><i class="fas fa-signal text-xl"></i></div>
                <div><span class="text-[10px] uppercase tracking-widest text-gray-500 block">Status</span><b
                        class="text-xl">Live</b></div>
            </div>
            <div class="bg-[#141414] border border-[#222] p-5 rounded-2xl flex items-center gap-4">
                <div class="bg-[#DFFF00]/5 p-3 rounded-xl text-[#DFFF00]"><i class="fas fa-bolt text-xl"></i></div>
                <div><span class="text-[10px] uppercase tracking-widest text-gray-500 block">System</span><b
                        class="text-xl">Fast</b></div>
            </div>
        </section>
        ...
        <main class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($products as $product)
                <article
                    class="bg-[#141414] border border-[#222] rounded-[24px] overflow-hidden group hover:border-[#DFFF00] transition-all flex flex-col relative h-fit">
                    @if($product->stock < 5)
                        <div
                            class="absolute top-4 left-4 bg-[#DFFF00] text-black text-[10px] font-extrabold px-3 py-1 rounded-md z-10 uppercase">
                            Low Stock</div>
                    @endif
                    ...
                    <div class="p-5 flex flex-col flex-grow">
                        <h4
                            class="text-sm font-semibold h-10 overflow-hidden line-clamp-2 mb-4 leading-snug group-hover:text-[#DFFF00] transition-colors">
                            {{ $product->name }}
                        </h4>
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-[#222]">
                            ...
                            <span
                                class="text-[10px] text-gray-400 border border-[#222] px-2 py-1 rounded-md font-bold uppercase tracking-tighter">QTY:
                                {{ $product->stock }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-[1fr_48px] gap-2 mt-4">
                            ...
                            <form method="POST" action="{{ route('seller.products.destroy', $product->id) }}"
                                class="contents">
                                @csrf @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete item?')"
