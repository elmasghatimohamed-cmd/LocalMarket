...
<div class="bg-darkBg min-h-screen py-12 font-sans text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col lg:flex-row gap-10">
            ...
            <div class="lg:w-1/3">
                <div class="bg-cardBg rounded-[2.5rem] p-8 border border-white/5 sticky top-8 shadow-2xl">
                    ...
                    @livewire('checkout-modal', ['cart' => $cart])
        ...
        <script>
            window.addEventListener('initStripe', event => {
            ...
            const cardElement = document.getElementById('modal-card-element');
            if (cardElement) {
                card.mount('#modal-card-element');
            }

            const btn = document.getElementById('modal-submit-payment');
            btn.replaceWith(btn.cloneNode(true)); 
            const newBtn = document.getElementById('modal-submit-payment');

            newBtn.addEventListener('click', async () => {
                newBtn.disabled = true;
                newBtn.innerHTML = "ENCRYPTING...";
                const {token, error} = await stripe.createToken(card);

                if (error) {
                    document.getElementById('modal-card-errors').textContent = error.message;
                    newBtn.disabled = false;
                    newBtn.innerHTML = "RETRY";
                } else {

                Livewire.dispatch('handleStripeToken', { token: token.id });
                }
            });
        }, 500); 
    });
