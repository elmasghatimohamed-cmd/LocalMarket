<nav x-data="{ open: false }" class="border-b border-white/5 bg-[#080808]/90 backdrop-blur-md sticky top-0 z-50">
    <div class="container mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8">
            ...
            @endrole
            @role('client')
            <x-nav-link href="{{ route('orders.index') }}" :active="request()->routeIs('seller.products.index')"

                class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#DFFF00]">
                {{ __('My Orders') }}
            </x-nav-link>
            <x-nav-link href="{{ route('favorites.index') }}" :active="request()->routeIs('seller.products.index')"
...
:active="request()->routeIs('seller.products.index')" class="text-gray-300"> My Products
